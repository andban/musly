...
bool write_jukebox(std::string &filename, musly_jukebox* jukebox, int last_reinit)
{
    ...
    {
        bool result = (musly_jukebox_tostream(jukebox, f) > 0) && (fwrite(&last_reinit, sizeof(last_reinit), 1, f) == 1);
        fclose(f);
        return result;
    }
    return false;
...
void tracks_add(collection_file &cf, std::string directory_or_file, std::string extension)
{
    fileiterator fi(directory_or_file, extension);
    std::string afile;
    if (!fi.get_nextfilename(afile))
    {
        std::cout << "No files found while scanning: " << directory_or_file << std::endl;
    }
    else
    {
        int buffersize = musly_track_binsize(mj);
    ...
    std::vector<musly_track *> &tracks)
{
    ...
    else
    {
        ...
        std::shuffle(tracks2.begin(), tracks2.end(), g);
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cout << "Version: " << musly_version() << std::endl;
        std::cout << "Available similarity methods: " << musly_jukebox_listmethods() << std::endl;
        std::cout << "Available audio file decoders: " << musly_jukebox_listdecoders() << std::endl;
#ifdef _OPENMP
    ...
    else
    {
        ...
        if (!jukebox_file.empty())
        {
            ...
            else
            {
                ...
                std::cout << "Jukebox file has " << num_new << " track(s) less than collection; updating..." << std::endl;
            ...
            {
                std::cout << std::endl
                          << "Initializing new jukebox..." << std::endl;
            }
        }
        else
        {
        ...
        if (po.get_action() == "e")
        {
            ...
            std::cout << "Correctly classified: " << genre_confusion.diagonal().sum()
                      << "/" << genre_confusion.sum() << " (" << ((float)genre_confusion.diagonal().sum() / (float)genre_confusion.sum()) * 100.0 << "%)" << std::endl;
